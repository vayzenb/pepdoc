...
% add project folders
data_dir = 'C:/Users/vayze/Documents/Research/Projects/PepDoc/data/raw/';
sub = 'AC';
...
%% Preprocesing
%Load data
sub_raw= [data_dir,'raw/' sub,'_raw.mff'];
EEG = pop_mffimport(sub_raw,'code');
...
% Rereferenced
    EEGrereferenced = pop_reref( EEG, EEG.ref); % pop up interactive window, rereference the average
...
% clean_raw data, but stronger than based on kurtosis. 
   pop_trimOutlier(EEG); % I have found this to be the best way to start
...
% Add conditions to trials BEFORE rejecting artifacts [Bird: 1, Insect: 2, Graspable Object/non-tool: 3, Tool: 4]
    ...
    cond_file= [data_dir, sub, '_ex1/conditionfile.txt'];
